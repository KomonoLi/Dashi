	 ...
	 private static final int MAX_RECOMMENDED_RESTAURANTS = 10;
	    ...
	    public JSONObject getRestaurantsById(String businessId, boolean isVisited) {
			...
			try {
				...
				//Document, can directly convert Java to JSON, but in MySQL,need to write class to convert by ourself 
	    ...
	    public JSONArray recommendRestaurants(String userId) {
	    	try {
				...
				Set<String> allCategories = new HashSet<>();// why hashSet? //step 2 //categories没有权重
	    ...
	    public JSONArray searchRestaurants(String userId, double lat, double lon, String term) {
	    	try {
				...
				for (int i = 0; i < array.length(); i++) {
					...
					//upsert : update(if we can find one) or insert(if we cant find BusinessId)
